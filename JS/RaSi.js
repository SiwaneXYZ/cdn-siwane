...
function isSafeUrl(e) {
    try { let t = new URL(e, location.href); return "https:" === t.protocol || "http:" === t.protocol } catch (e) { return !1 }
}
...
function renderRichText(e) {
    ...
    t = t.replace(/^[*\-]\s+(.*)$/gm, (e, t) => {
        n++; let s = n <= 10 ? ["١", "٢", "٣", "٤", "٥", "٦", "٧", "٨", "٩", "١٠"][n - 1] : n + ".";
        return `${s} ${t.trim()}<br>`
    });
    t = t.replace(/(?<!\w)[*#](?!\w)/g, "");
...
function initUsage() {
    let e = new Date().toISOString().slice(0, 10), t = { date: e, count: 0, limit: DEFAULT_DAILY_LIMIT };
...
function refreshUsageUI() {
    ...
    if (remaining === Infinity) {
        ...
        remItem.classList.add("limited"); remItem.classList.remove("unlimited"); remItem.title = `${remaining} رسائل متبقية`;
    }
}
...
function createUserMessage(e) {
    ...
    t.appendChild(n);
    let s = document.createElement("div"); s.className = "meta";
    s.innerHTML = `<div class="msg-controls"><button class="edit-user" title="تعديل"><svg viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg></button></div>`;
...
function createAiPlaceholder() {
    ...
    let t = document.createElement("div"); t.className = "bubble";
...
function buildConversationPayload(e) {
    ...
    htmlMessages.forEach(el => {
        let isUser = el.classList.contains("RaSi-msg-user");
...
async function sendMessage(e, t = null, n = !1) {
    ...
    if (!isDev) {
        let usage = loadUsage();
        if (usage.count >= usage.limit) return showStatus("تم تجاوز الحد اليومي للرسائل"), !1;
    }
    ...
    try {
        ...
        let replyText = data.reply || "✗ حدث خطأ.";
        ...
        refreshUsageUI(); saveHistory(); showStatus("تم الرد بنجاح!");
        return !0;
    } catch (error) {
        let bubbleElement = placeholder.querySelector(".bubble");
        ...
        if(retryBtn) {
            ...
            retryBtn.onclick = async function() {
                ...
                await sendMessage(e, placeholder, !0);
                retryBtn.disabled = !1;
            };
        }
...
document.getElementById("RaSi-chat-btn").onclick = function() {
    container.style.display = "flex"; container.style.position = "fixed"; container.style.left = ""; container.style.top = ""; container.style.right = "32px"; container.style.bottom = "142px";
    lazyLoadMessages();
...
txt.addEventListener("keydown", function(e) {
    if (("Enter" === e.key && !e.shiftKey) || ((e.ctrlKey || e.metaKey) && "Enter" === e.key)) {
        e.preventDefault(); document.getElementById("RaSi-send").click(); return;
    }
});
...
document.getElementById('RaSi-messages').addEventListener('click', function(e) {
    ...
    if (target.classList.contains('copy-reply')) {
        ...
        navigator.clipboard.writeText(text).then(() => {
            ...
            const originalBg = target.style.background; const originalColor = target.style.color;
...
document.getElementById("RaSi-send").onclick = async function() {
    ...
    if(!isUnlimited) {
        let usage = loadUsage();
        if(usage.count >= usage.limit) { showStatus("تم تجاوز الحد اليومي للرسائل"); return; }
    }
...
function ensureFullMessageVisibility() {
    ...
    if (lastMessage) {
        const messageHeight = lastMessage.offsetHeight; const containerHeight = messagesContainer.offsetHeight;
...
head.addEventListener("click", function() {
    ...
    if (headerClickCount >= 5) {
        ...
        if (t) { localStorage.removeItem(DEV_FLAG_KEY); showStatus("وضع المطور معطل"); } else { localStorage.setItem(DEV_FLAG_KEY, "1"); showStatus("وضع المطور مفعل: غير محدود"); }
        refreshUsageUI();
    }
});
