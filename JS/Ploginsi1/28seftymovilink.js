document.addEventListener("DOMContentLoaded", function() {
    ...
    async function loadSeriesLobby(sheet, container, config) {
        ...
        try {
            ...
            container.innerHTML = `<div class="siwane-container"><p class="note">حدث خطأ في تحميل القائمة.</p></div>`;
        }
    }
    ...
    async function playSelectedServer(serverId, params) {
        ...
        try {
            ...
            countdownText.innerHTML = "خطأ في تحميل السيرفر. يرجى المحاولة مرة أخرى.";
        }
    }
    ...
    async function loadServers(params) {
        ...
        try {
            ...
            servers.forEach(s => {
                ...
                btn.dataset.id = s.id;
                btn.innerHTML = `
                    <span style="margin-left: 5px;">${s.icon || '🔗'}</span>
                    <span>${s.title || 'سيرفر مشاهدة'}</span>
                `;
    ...
    // دالة للتحقق من حالة Worker
    async function checkWorkerStatus() {
        try {
            const response = await fetch(WORKER_URL);
            const text = await response.text();
            console.log('Worker status:', response.status, text.substring(0, 50));
            return response.ok;
        } catch (error) {
            console.error('Worker is not reachable:', error);
            return false;
        }
    }
    
    // دالة للتحقق من اتصال GAS
    async function checkGASStatus(gasUrl) {
        try {
            const testUrl = `${gasUrl}?contentSheetName=test&action=getEpisodes`;
            const response = await fetch(testUrl);
            return response.ok;
        } catch (error) {
            console.error('GAS is not reachable:', error);
            return false;
        }
    }
    
    // التحقق من الاتصالات عند التحميل
    window.addEventListener('load', async function() {
        if (config.GAS_URL) {
            const gasOk = await checkGASStatus(config.GAS_URL);
            if (!gasOk) {
                console.warn('GAS connection issue detected');
            }
        }
        
        const workerOk = await checkWorkerStatus();
        if (!workerOk) {
            console.error('Worker connection issue detected');
            
            // عرض رسالة للمستخدم إذا كان في وضع المشاهدة
            if (mode === 'watch') {
                const errorMsg = document.createElement('div');
                errorMsg.innerHTML = `
                    <div class="siwane-container" style="background:#ffebee;border:1px solid #ffcdd2;">
                        <p style="color:#c62828;text-align:center;">
                            ⚠️ مشكلة في الاتصال بخادم المشغل. يرجى المحاولة لاحقاً أو التواصل مع الدعم.
                        </p>
                    </div>
                `;
                
                const container = document.querySelector(".siwane-container");
                if (container) {
    ...
    document.addEventListener('keydown', function(e) {
        // مفتاح المسافة للتشغيل/الإيقاف (إذا كان الفيديو مرئياً)
        if (e.code === 'Space' && mode === 'watch') {
            const videoFrame = document.getElementById('siwane-video-frame');
            if (videoFrame && videoFrame.style.display !== 'none') {
                e.preventDefault();
                // يمكن إضافة منطق للتحكم في الفيديو هنا
            }
        }
        
        // مفتاح F للملء الشاشة
        if (e.code === 'KeyF' && mode === 'watch') {
            const videoFrame = document.getElementById('siwane-video-frame');
            if (videoFrame && videoFrame.style.display !== 'none') {
                e.preventDefault();
    ...
    // إضافة تحسينات لتحميل الصور
    function preloadIcons() {
        const iconUrls = [
            // يمكن إضافة أي أيقونات إضافية هنا
        ];
    ...
    // استدعاء تحميل الأيقونات مسبقاً
    ...
    if ('ontouchstart' in window) {
        ...
        // زيادة حجم أزرار اللمس على الأجهزة المحمولة
        const style = document.createElement('style');
        style.textContent = `
            @media (max-width: 768px) {
                .siwane-episode-btn, .siwane-server-btn {
