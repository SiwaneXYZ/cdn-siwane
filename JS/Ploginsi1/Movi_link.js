document.addEventListener("DOMContentLoaded", function() {
    // 1. الإعدادات العامة - نفس النظام بالضبط
    ...
    // 🔐 دالة لتحميل مكتبة التشفير ديناميكياً
    ...
    async function decryptWorkerUrl(encrypted) {
        ...
        try {
            ...
            console.error("فك تشفير Worker فشل:", e);
            return null;
        }
    }
    ...
    // ==========================================
    // 🛡️ حماية اللوبي (Vanilla JS) - نفس النظام السابق بالضبط
    // ==========================================
    ...
    // ==========================================
    // 📺 جلب المحتوى (Fetch API) - نفس النظام السابق
    // ==========================================
    ...
    // ==========================================
    // 🎥 المشغل والعداد - تحديث بسيط فقط للتشفير
    // ==========================================
            ...
            if (res.realUrl) {
                if (res.encrypted) {
                    ...
                    if (realUrl) {
                        ...
                        countdownText.innerHTML = "خطأ في فك تشفير الرابط";
                    }
                } else {
                ...
                countdownText.innerHTML = "لم يتم العثور على رابط الفيديو";
            }
        } catch (error) {
            console.error(error);
    ...
    // ==========================================
    // بناء واجهة صفحة المشاهدة - لا تغيير
    // ==========================================
    ...
    // 🔐 دالة إنشاء المشغل الآمن (محدثة للتشفير)
    function createSecurePlayer(realUrl) {
        // إخفاء الرابط في Blob بشكل آمن
        const html = `
        <!DOCTYPE html>
        <html>
        <head>
    ...
    // دالة قديمة للتوافق مع النظام القديم
