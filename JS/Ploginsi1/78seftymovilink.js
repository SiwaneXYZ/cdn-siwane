document.addEventListener("DOMContentLoaded", function() {
    ...
    // ==========================================
    // 🎥 المشغل والعداد (التحديث الرئيسي هنا)
    // ==========================================
    ...
    async function playSelectedServer(serverId, params) {
        ...
        try {
            // 🔄 التحديث: جلب HTML مباشرة من الـ Worker
            const response = await fetch(
                `${WORKER_URL}/get-secure-player?sheet=${encodeURIComponent(params.SHEET)}&id=${encodeURIComponent(serverId)}&_=${Date.now()}`
            );
            ...
            // قراءة الـ HTML كـ Blob مباشرة
            const blob = await response.blob();
            const blobUrl = URL.createObjectURL(blob);
            
            // بدء العداد مع رابط الـ Blob
            startCountdownAndAds(blobUrl, params);
            ...
            countdownText.innerHTML = "⚠️ خطأ في تحميل السيرفر. جرب سيرفر آخر.";
            
            // إعادة تمكين الأزرار بعد الخطأ
            setTimeout(() => {
                document.querySelectorAll(".siwane-server-btn").forEach(btn => {
    ...
    function startCountdownAndAds(blobUrl, params) {
        let count = params.COUNTDOWN;
        const countdownEl = document.getElementById("siwane-countdown");
        countdownEl.textContent = count;
        
        // إخفاء أزرار السيرفر أثناء التحميل
        document.querySelectorAll(".siwane-server-btn").forEach(btn => {
    ...
    // 🛡️ دالة إنشاء مشغل آمن (للتوافق مع الكود القديم)
    function createSecurePlayer(enc) {
        return URL.createObjectURL(new Blob([`
            ...
            <head>
                ...
                <script>
                    // تشفير بسيط للتوافق
                    const url = atob("${enc}".split('').reverse().join(''));
