$(document).ready((function() {
    // 1. الإعدادات والبارامترات العامة
    ...
    const formatTitle = (text) => {
        if (!text) return "";
        return text.trim().replace(/^مسلسل\s+/i, "");
    };
    ...
    if ("watch" === mode && canViewContent) {
        ...
        // تفعيل اللوبي (الخمول الآمن)
        initializeLobbyWithProtection(config);
    }

    // ==========================================
    // 🛡️ الجزء الأول: حماية اللوبي (تفاعل بشري + تصفية عناوين)
    // ==========================================
    function initializeLobbyWithProtection(config) {
        ...
        const cleanSheetName = formatTitle(rawSheet);

        let actionText, headerText, scrollContext;
        if (movie) {
            headerText = `بوابة فيلم: ${movie}`;
            actionText = `<i class="fa fa-play-circle"></i> بدء مشاهدة الفيلم`;
            scrollContext = "رابط الفيلم";
        } else {
            headerText = `حلقات مسلسل: ${cleanSheetName}`;
            actionText = `<i class="fa fa-list-ul"></i> استعراض الحلقات`;
            ...
            <div class="siwane-container" id="siwane-auth-wrapper">
                <div class="siwane-server-container" style="text-align:center;">
                    ...
                    <p id="scroll-msg" style="display:none; color: #d35400; font-weight: bold; font-size: 13px;">
                        <i class="fa fa-mouse-pointer"></i> يرجى التمرير للأسفل قليلاً لتأمين ${scrollContext}...
                    </p>
                </div>
            </div>
        `);
        ...
        $("#activate-trigger").click(function(e) {
            e.preventDefault();
            $(this).fadeOut(200);
            $("#activation-status").text("جاري التحقق من الهوية...");
            $("#scroll-msg").fadeIn();
            ...
            $(window).on('scroll.siwaneAuth', function() {
                if (!scrollTriggered) {
                    ...
                    $("#scroll-msg").html('<i class="fa fa-spinner fa-spin"></i> جاري استدعاء البيانات (2 ثانية)...');
                    setTimeout(function() {
    ...
    // ==========================================
    // 📺 الجزء الثاني: جلب الحلقات (دعم كلمة "الأخيرة")
    // ==========================================
    function loadSeriesLobby(sheet, container, config) {
        const cleanSheetName = formatTitle(sheet);
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.episodes && response.episodes.length > 0) {
                    let html = `<div class="siwane-container"><div class="siwane-episodes-container"><h2>حلقات ${cleanSheetName}</h2><div class="siwane-episodes-grid">`;
                    response.episodes.forEach(episode => {
                        if (episode !== null) { 
                            // تم السماح بالنصوص والأرقام معاً (حل مشكلة كلمة "الأخيرة")
    ...
    function loadMovieLobby(sheet, movieTitle, container, config) {
        container.html(`<div class="siwane-container"><div class="siwane-episodes-container"><h2>${movieTitle}</h2><div class="siwane-episodes-grid" style="grid-template-columns: 1fr;"><div class="siwane-episode-btn" onclick="siwaneRedirect('${sheet}', '${movieTitle}', 'movie')"><i class="fa fa-play"></i> شاهد الآن</div></div></div></div>`);
        window.siwaneRedirect = (s, t, ty) => redirectToWatchPage(s, t, ty);
    }

    // ==========================================
    // 🔗 الجزء الثالث: التحويل والذاكرة الذكية
    // ==========================================
    async function redirectToWatchPage(sheet, id, type) {
        try {
            ...
            if (data.feed.entry) {
                ...
                window.location.href = `${postUrl}${sep}mode=watch&sheet=${encodeURIComponent(sheet)}${idParam}`;
            }
    ...
    function handleWatchRoute() {
        ...
        if (sheet && config.GAS_URL) {
            ...
            // استرجاع السيرفر المختار تلقائياً عند العودة
            const lastSession = sessionStorage.getItem("siwane_last_server");
            if (lastSession) {
                const data = JSON.parse(lastSession);
                // نتحقق أن المستخدم عاد لنفس الحلقة/الفيلم
                if (data.sheet === params.SHEET && data.id === params.ID) {
                    setTimeout(() => {
                        $(`.siwane-server-btn[data-id="${data.serverId}"]`).trigger('click');
                    }, 1200);
                }
            }
        }
    ...
    function playSelectedServer(serverId, params) {
        ...
        // حفظ السيرفر المختار في الذاكرة المؤقتة
        sessionStorage.setItem("siwane_last_server", JSON.stringify({
            sheet: params.SHEET, id: params.ID, serverId: serverId
        }));
    ...
    // --- بقية دوال الواجهة (Particles, AdGate, Blob) ---
    function initializeWatchPage(params) {
        ...
        contentContainer.prepend(`
            <div class="siwane-container"><header class="siwane-header"><h1>${title}</h1></header>
            <div class="siwane-server-container"><h2>اختر سيرفر المشاهدة</h2><div id="siwane-servers-grid" class="siwane-servers-grid loading-state"><p>جاري تحميل السيرفرات...</p></div></div></div>
        `);
        contentContainer.append(`
            <div class="siwane-container"><div class="siwane-video-container"><h2>شاشة العرض</h2>
            <div id="siwane-countdown-display" style="display:none;"><div id="siwane-particles-container" class="siwane-particles-container"></div><div id="siwane-countdown-text"></div><div id="siwane-countdown"></div></div>
            ...
            <a class="button ln" href="/p/offerwal.html" style="width:100%;text-align:center;display:block;margin-top:10px;">ادعمنا بالنقر هنا واستمر بالمشاهدة</a></div></div>
        `);
        loadServers(params);
    ...
    function loadServers(params) {
        ...
        let q = `contentSheetName=${encodeURIComponent(params.SHEET)}&${params.TYPE==="movie" ? `movieTitle=${encodeURIComponent(params.ID)}` : `episodeNumber=${params.ID}`}`;
        $.ajax({
            ...
            success: function(servers) {
                ...
                servers.forEach(s => {
                    ...
                    btn.click(function() { $(".siwane-server-btn").removeClass("active"); $(this).addClass("active"); $("html,body").animate({scrollTop:$(".siwane-video-container").offset().top-20},600); playSelectedServer(s.id, params); });
                    grid.append(btn);
                });
            }
        });
