...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function getUserDataFromStorage() {
        ...
        if (dataString) {
            try {
                ...
                if (parsedData && typeof parsedData === 'object' && parsedData.uid) { // تم التعديل هنا
                    userData = parsedData; // تم التعديل هنا
    ...
    function simulateLogin(userDataObj) { // تم التعديل هنا (اسم البارامتر لم يتغير لكن المنطق يتوقع هيكل بيانات مختلف الآن)
        console.warn('Using simulated login. This does NOT authenticate with Firebase.');
        if (typeof userDataObj === 'object' && userDataObj !== null && userDataObj.uid) { // تم التعديل هنا (نتوقع الآن uid مباشرة وليس userData.uid)
            localStorage.setItem(FIREBASE_PROFILE_STORAGE_KEY, JSON.stringify(userDataObj)); // تم التعديل هنا (نحفظ الكائن المقدم مباشرة)
            updateUI();
            if (loginCheckbox) {
                 loginCheckbox.checked = false;
            }
