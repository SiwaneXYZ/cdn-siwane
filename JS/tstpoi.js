  ...
  document.addEventListener('DOMContentLoaded', () => {
      ...
      notificationElement = document.getElementById(articleAppConfig.notificationId); // حاول العثور عليه أولاً إذا كان موجوداً بالفعل
      ...
      if (flowStarted === 'true' && startTimeString) {
          ...
          if (isNaN(startTime)) {
              console.error("Article Page: startTime from localStorage is not a valid number. Clearing flags and exiting flow.");
               ...
               if (notificationElement) notificationElement.style.display = 'none'; // إخفاء إذا كان موجوداً
              return;
          }

           // *** إنشاء الإشعار إذا لم يتم العثور عليه بالفعل ***
           ...
           durationCheckInterval = setInterval(() => {
               ...
               if (timeSpent >= articleAppConfig.requiredDurationMillis) {
                   ...
                   // *** تسجيل حالة جميع علامات المقال قبل مسح علامات البدء والإعادة التوجيه ***
                    console.log(`Article Script: localStorage state BEFORE clearing start flags and redirect:`, { ...localStorage }); // نسخ وقراءة جميع المفاتيح
                   ...
                   redirectTimeout = setTimeout(() => {
                       ...
                       console.log("Article Script: Article start localStorage flags cleared."); // تأكيد
    ...
    function handleBeforeUnload() {
        ...
        if (flowStarted === 'true' && startTimeString && completed !== 'true') {
         ...
         // *** تسجيل حالة جميع علامات المقال قبل مسح علامات البدء في beforeunload ***
