...
function isSafeUrl(e) { try { let t = new URL(e, location.href); return "https:" === t.protocol || "http:" === t.protocol } catch (e) { return !1 } }
...
function showStatus(e, t = 1600) { let n = document.getElementById("RaSi-status"); n.style.display = "block"; n.textContent = e; t > 0 && setTimeout(() => { n.style.display = "none" }, t); }
...
function createUserMessage(e) {
    ...
    t.appendChild(s); document.getElementById("RaSi-messages").appendChild(t); return t;
}
...
function createAiPlaceholder() {
    ...
    e.appendChild(n); document.getElementById("RaSi-messages").appendChild(e); setTimeout(() => { ensureFullMessageVisibility(); }, 100); return e;
}
...
function buildConversationPayload(e) {
    ...
    htmlMessages.forEach(el => {
        ...
        parsedMessages.push({ role: isUser ? "user" : "assistant", content: textContent });
    });
...
async function sendMessage(e, t = null, n = !1) {
    ...
    if (!isDev) { let usage = loadUsage(); if (usage.count >= usage.limit) return showStatus("تم تجاوز الحد اليومي للرسائل"), !1; }
    ...
    try {
        ...
        let replyText = data.reply || "✗ حدث خطأ.", bubbleElement = placeholder.querySelector(".bubble");
        if(bubbleElement) bubbleElement.innerHTML = renderRichText(replyText);
        
        let retryBtn = placeholder.querySelector(".resend-retry");
        if(retryBtn) retryBtn.style.display = "none";
        
        let u = loadUsage(); u.count = (u.count || 0) + 1; saveUsage(u); refreshUsageUI(); saveHistory(); showStatus("تم الرد بنجاح!"); return !0;
    } catch (error) {
        let bubbleElement = placeholder.querySelector(".bubble");
        if(bubbleElement) bubbleElement.innerHTML = `<div style="color:#ef4444;">✗ تعذر استجابة المساعد</div>`;
        let retryBtn = placeholder.querySelector(".resend-retry");
        if(retryBtn) {
            retryBtn.style.display = "inline-block";
            retryBtn.onclick = async function() { retryBtn.disabled = !0; bubbleElement.innerHTML = `<div style="display:flex;align-items:center;gap:8px;"><div class="spinner"></div> إعادة المحاولة...</div>`; await sendMessage(e, placeholder, !0); retryBtn.disabled = !1; };
        }
        saveHistory(); showStatus("تعذر الاتصال بالخادم"); return !1;
    }
}
...
function lazyLoadMessages() {
    if (!messagesLoaded) {
        ...
        e.appendChild(n); document.getElementById("RaSi-messages").appendChild(e); messagesLoaded = !0;
        setTimeout(() => { document.getElementById("RaSi-messages").scrollTop = document.getElementById("RaSi-messages").scrollHeight; }, 100);
    }
}
...
document.getElementById('RaSi-fullscreen').onclick = function() { if (container.classList.contains('RaSi-fullscreen')) { exitFullscreenMode(); } else { enterFullscreenMode(); } };
...
document.addEventListener("keydown", function(e) { if ("Escape" === e.key) container.style.display = "none"; });
document.getElementById("RaSi-copy-all").onclick = function() { let e = [...document.getElementById("RaSi-messages").children].map(e => e.innerText).join("\n"); navigator.clipboard.writeText(e).then(() => showStatus("تم نسخ المحادثة!")); };
document.getElementById("RaSi-clear").onclick = function() { localStorage.removeItem(HISTORY_KEY); document.getElementById("RaSi-messages").innerHTML = ""; messagesLoaded = !1; lazyLoadMessages(); showStatus("تم حذف المحادثة!"); };
...
document.getElementById("RaSi-send").onclick = async function() {
    ...
    setTimeout(() => { const messagesContainer = document.getElementById("RaSi-messages"); messagesContainer.scrollTop = messagesContainer.scrollHeight; }, 50);
    saveHistory(); await sendMessage(messageText, aiMessage);
    setTimeout(() => { const messagesContainer = document.getElementById("RaSi-messages"); messagesContainer.scrollTop = messagesContainer.scrollHeight + 100; }, 100);
};

function ensureFullMessageVisibility() {
    const messagesContainer = document.getElementById("RaSi-messages"); const lastMessage = messagesContainer.lastElementChild;
    if (lastMessage) { const messageHeight = lastMessage.offsetHeight; const containerHeight = messagesContainer.offsetHeight; messagesContainer.scrollTop = messagesContainer.scrollHeight - containerHeight + messageHeight + 20; }
}
...
document.addEventListener("click", function(e) {
    ...
    if("flex" !== t.style.display || t.contains(e.target) || n.contains(e.target)) return; t.style.display = "none";
});
