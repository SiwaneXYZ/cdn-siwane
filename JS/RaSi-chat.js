document.addEventListener("DOMContentLoaded", function() {
    ...
    const USAGE_KEY = "RaSiChatUsage_v1",
          ...
          // جلب الليميت من HTML إذا كان موجوداً، وإلا نستخدم 25 كافتراضي
    ...
    // =====================================================================
    // 1.5. الحماية والتخصيص (النطاق والتصنيفات)
    // =====================================================================
    
    // أ. حماية النطاق (التأكد من أن السكريبت يعمل فقط على siwane.xyz)
    const hostname = window.location.hostname;
    const isAuthorizedDomain = hostname === 'siwane.xyz' || hostname.endsWith('.siwane.xyz') || hostname === 'localhost';
    
    if (!isAuthorizedDomain) {
        console.warn("RaSi Chat: تم إيقاف السكريبت، النطاق غير مصرح به.");
        chatBtn.style.display = "none";
        return; 
    }

    // ب. التحقق من التصنيفات (خاص بمدونات بلوجر)
    if (typeof EXCLUDED_CATEGORIES !== 'undefined' && Array.isArray(EXCLUDED_CATEGORIES) && EXCLUDED_CATEGORIES.length > 0) {
        ...
        let isExcluded = pageTags.some(tag => EXCLUDED_CATEGORIES.includes(tag));
        
        if (isExcluded) {
            console.log("RaSi Chat: تم إخفاء الزر لأن تصنيف المقال ضمن القائمة الممنوعة.");
            chatBtn.style.display = "none";
            return; 
        }
    }
    ...
    function loadUsage() { 
        try { 
            ...
            // تحديث الليميت فوراً إذا تغير في HTML
            if (t.limit !== DEFAULT_DAILY_LIMIT) {
                t.limit = DEFAULT_DAILY_LIMIT;
                saveUsage(t);
            }
            return t; 
        } catch (s) { 
