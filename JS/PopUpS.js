...
// يعتمد هذا السكربت على وجود عناصر HTML محددة في الإصدار غير AMP من الصفحة.
...
(function() {
    ...
    // نتحقق من مؤشرات AMP الشائعة في بداية تنفيذ السكربت.
    if (document.documentElement.hasAttribute('amp') || document.querySelector('link[rel="amphtml"]')) {
        ...
        return; // إيقاف التنفيذ فوراً في صفحات AMP
    }
    // --- نهاية فحص AMP ---
    ...
    // هذا قد يفشل إذا لم يتم تهيئة 'app' بسبب خطأ سابق، ولكن 'return' السابق يتعامل مع ذلك.
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (!isUserLi) {
            ...
            console.error(
                ...
                'لن يتم تحديث واجهة المستخدم. يرجى التأكد من وجود هذا العنصر في الشفرة المصدرية النهائية لصفحة HTML (استخدم أدوات المطور)، ' +
                'خاصة عند فحص شروط قالب بلوجر التي قد تخفيه عن الإصدار غير AMP.' // توضيح أكبر بخصوص بلوجر
            );
        ...
        // يتم البحث عن popupCheckbox باستخدام ID عالمي، أو يمكن حصره داخل isUserLi.querySelector('#forlogPop')
        ...
        // نعيد البحث هنا للتأكد من أنه ضمن نطاق DOMContentLoaded وأن donLogDiv موجود
        const logoutButton = donLogDiv ? donLogDiv.querySelector('div[aria-label="الخروج"]') : null;


        // التحقق من وجود جميع العناصر الضرورية بعد العثور على العنصر الأساسي
        ...
        onAuthStateChanged(auth, (user) => {
            ...
            if (user) {
                ...
                // 3. تأكد من إغلاق النافذة المنبثقة عند تسجيل الدخول الناجح (قد تكون مفتوحة من محاولة سابقة)
        ...
        if (logoutButton) { // نستخدم متغير logoutButton الذي تم البحث عنه في الأعلى ضمن نطاق DOMContentLoaded
            ...
            logoutButton.addEventListener('click', () => {
                // استدعاء دالة signOut لتسجيل خروج المستخدم من Firebase
                signOut(auth).then(() => {
                    console.log('تم تسجيل خروج المستخدم بنجاح من Firebase.');

                    // === إضافة التحويل هنا بعد تسجيل الخروج بنجاح ===
                    // يمكنك التحويل إلى صفحة تسجيل الدخول أو أي صفحة أخرى
                    // هذا يحاكي سلوك دالة logOut() القديمة ولكن باستخدام آلية Firebase
